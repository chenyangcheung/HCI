<?php get_header(); ?>
    <link href="css/schedulelist.css" type="text/css" rel="stylesheet">
</head>
<body>
    <nav class="hci-nav hasbg" id="hciNav">
        <div class="hci-nav-inner">
<?php get_headlink(); ?>
            <?php get_schedule(); ?>
            <?php get_searchform(); ?>
        </div>
    </nav>
    <header class="hci-nav-bg">
        <div class="hci-overlay"></div>
        <h1><span>重要日</span>程</h1>
        <h2><span>武汉大学人机交互与用户行为研究中心</span></h2>
    </header>
    <section class="hci-schedule-body">
        <ul class="hci-res-list">
            <?php 
                while ( have_posts() ) : the_post();
                    $post_expert = get_the_excerpt();
                    $post_thumb = get_the_post_thumbnail_url();
            ?>
            <li class="hci-sche-item">
                <img src="<?php 
                            if(!$post_thumb) {
                                echo 'img/pro1.jpg';
                            } else {
                                the_post_thumbnail_url();
                            } ?>" class="hci-sche-img">
                <div class="hci-sche-content">
                    <div class="hci-sche-header">
                        <span class="hci-sche-date"><?php the_time('Y年m月j日'); ?></span>
                    </div>
                    <a class="hci-sche-title" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
                    <div class="hci-sche-abs">
                            <?php 

                                if(!$post_expert) {
                                    echo '在这里输入摘要，字数在100到140之间。在这里输入摘要，字数在100到140之间。在这里输入摘要，字数在100到140之间。在这里输入摘要，字数在100到140之间。在这里输入摘要，字数在100到140之间。在这里输入摘要，字数在100到140之间。在这里输入摘要，字数在100到140之间。';
                                } else {
                                    the_excerpt();
                                } 
                            ?>
                    </div>
                </div>
            </li>
            <?php
                endwhile;
            ?>
        </ul>
        <nav class="hci-pagination">
            <ul class="hci-pagination-list" id="hciPagination">
                <?php par_pagenavi(2); ?>

            </ul>
        </nav>
<?php get_footer(); ?>
    </section>
    <div class="mask"></div>
    <script src="js/lib/require.js" data-main="js/research.js"></script>
</body>
</html>